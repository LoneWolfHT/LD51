...
func _ready():
	...
	if (signal_target != self && self.connect("pressed", signal_target, "_on_button_activate", [ button_name ]) != OK):
		push_error("[util->buttonpress]: Failed to connect signal to given node")

	if load_scene:
		if (self.connect("pressed", self, "_on_loadscene_request", [ load_scene ]) != OK):
